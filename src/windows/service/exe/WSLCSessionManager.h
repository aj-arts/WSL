    ...
    inline auto ForEachSessionLockHeld(const auto& Routine, std::vector<SessionEntry>& DeadSessions)
    {
        ...
        auto each = [&](SessionEntry& entry) {
            ...
            if (FAILED_LOG(entry.Ref->OpenSession(&lockedSession)))
            {
                ...
                try
                {
                    // Reserve the slot up front so push_back cannot reallocate
                    // (and therefore cannot throw) after `entry` is moved-from.
                    // Combined with SessionEntry's noexcept move, this guarantees
                    // `entry` is only consumed once the destination slot is known
                    // to exist. reserve() is inside the try so an allocation
                    // failure there cannot escape the remove_if predicate either.
